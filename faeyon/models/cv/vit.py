...
class ViT(nn.Module):
    ...
    def forward(
        ...
        if channels != self.num_channels:
            raise ValueError(
                f"Make sure that the number of image channels match with the one set in"
                f" the class initialization. Expected {self.num_channels} but got {channels}."
            )

        embeddings = img >> self.patch_embedding >> self.flatten
        embeddings = embeddings.transpose(-1, -2)  # (B, P, E)
